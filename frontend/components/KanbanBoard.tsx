...
function KanbanCard({ pedido }: { pedido: any }) {
...
function KanbanColumn({ id, title, icon: Icon, color, pedidos, limitInfo }: { id: string, title: string, icon: any, color: string, pedidos: any[], limitInfo?: string }) {
  const { setNodeRef, isOver } = useDroppable({ id });

  return (
    <div ref={setNodeRef} className={`flex flex-col bg-slate-100/50 dark:bg-[#1C1C1E]/50 rounded-[24px] p-4 border-2 transition-colors ${isOver ? `border-${color}-500 bg-${color}-50 dark:bg-${color}-900/10` : 'border-transparent'}`}>
...
export default function KanbanBoard({ periodo }: { periodo: string }) {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const statusAntigo = active.data.current?.statusAtual;
    ...
    <DndContext onDragEnd={handleDragEnd}>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <KanbanColumn 
          ...
          /* AQUI ESTÁ A LIMITAÇÃO PARA NÃO LOTAR A TELA */
