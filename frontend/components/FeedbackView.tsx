...
export default function FeedbackView({ periodo = '7dias' }: { periodo?: string }) {
  ...
  // Função central para buscar dados e solicitar análise de IA
  async function carregarDados() {
    ...
    try {
      ...
      setInsights([{
        ...
        reclamacao: lang === 'en' ? "Could not connect to database." : "Não foi possível conectar ao banco.",
        dica: lang === 'en' ? "Check backend." : "Verifique o backend."
      }]);
    } finally {
  ...
  // Recarrega sempre que o período mudar no Dashboard
  useEffect(() => {
    carregarDados();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-end mb-8">
        ...
        >
          <Plus size={18} /> {t.feedbacks.newSimulation}
        </button>
      </div>
